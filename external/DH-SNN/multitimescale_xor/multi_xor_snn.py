"""
Multi‑timescale XOR (multi_xor_snn.py): 
one initial signal (signal 1) appears early, 
then multiple signal‑2 segments appear later 
at fixed intervals with gaps. 
The target is evaluated during each signal‑2 segment, 
so the model must combine the initial signal 
with repeated later signals across multiple timescales.
"""
    ...
    def forward(self,input,target):
        ...
        for i in range(seq_num):
            ...
            if (((i-start_time) % (coding_time+remain_time))> remain_time)and(i>start_time):
                ...
                loss += self.criterion(l2_output, target[:, i].long())
                output = F.softmax(l2_output, dim=1)  # acc 계산용으로만
...
model = Dense_test_1layer(input_size=channel_size*2, hidden_dims=hidden_dims, output_dim=output_dim)
